#let briefpapier(
  ...
  // Absender
  let final-absender = if show-absender [#place(right + top, dy: -10mm, absender)] else []
  block(width: 100%, height: 15mm, spacing: 0mm, final-absender)
  ...
  block(width: 100%, height: 3em, spacing: 0mm)[
    #set align(top + right)
    #ort, #final-datum
  ]
  ...
  // Betreff
  block(width: 100%, height: 2em, spacing: 0mm)[
